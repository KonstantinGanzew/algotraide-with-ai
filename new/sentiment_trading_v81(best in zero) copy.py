...
logging.basicConfig(
    ...
    filename='trading_v13_8.log', # ✅ ИЗМЕНЕНИЕ: Новый файл лога
    filemode='w'
)
...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.8 - СТИМУЛИРУЕМ УДЕРЖАНИЕ ПРИБЫЛИ
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. ATR_TP_MULTIPLIER: Возвращено значение 4.0 для восстановления высокого Win Rate.
       2. REWARD_SYSTEM: Внедрен "бонус за жадность" (holding_incentive) в среде.
    """
    ...
    ATR_TP_MULTIPLIER = 4.0       # ✅ ИЗМЕНЕНИЕ: Возвращаем к 4.0, чтобы восстановить качество входов
...
class TradingEnv(gym.Env):
    ...
    def step(self, action: int):
        try:
            ...
            # ✅ ИЗМЕНЕНИЕ V13.8: ВНЕДРЕНИЕ "БОНУСА ЗА ЖАДНОСТЬ"
            # Мы даем агенту небольшое положительное вознаграждение за каждый шаг,
            # который он находится в прибыльной позиции. Это должно мотивировать его
            # не закрывать сделку преждевременно, а дождаться тейк-профита.
            holding_incentive = 0.0
            if unrealized_pnl > 0:
                # Коэффициент 0.05 - это настраиваемый "коэффициент жадности".
                # Он не должен быть слишком большим, чтобы не перебить сигнал от реальной прибыли.
                holding_incentive = (unrealized_pnl / self.cfg.INITIAL_BALANCE) * 0.05
            
            reward = (realized_reward / self.cfg.INITIAL_BALANCE) + holding_incentive - transaction_penalty
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.8 (Бонус за жадность) " + "="*20)
        ...
        print("\n🎓 ЭТАП 4: ОБУЧЕНИЕ 'ЖАДНОГО, НО УМНОГО АНАЛИТИКА'..."); 
...
# 📊 ЭТАП 6: АНАЛИЗ РЕЗУЛЬТАТОВ
# ============================================================
# 💰 Финальный баланс: $9,641.04 (Начальный: $10,000.00)
# 📈 Доходность стратегии: -3.59%
# 📊 Доходность Buy & Hold: +16.73%
# ------------------------------------------------------------
# 📈 Детальная статистика PnL:
#    🟢 Валовая прибыль: $1,102.97
#    🔴 Валовый убыток:  $-1,168.77
#    💸 Всего комиссий:  $586.13
#    ⚖️ Профит-фактор:    0.94
# ------------------------------------------------------------
# 🛡️ Статистика Риска и Дисциплины:
#    📉 Макс. просадка:   -8.94%
#    ⏳ Средняя длительность сделки: 5.51 часов
#    🏆 Соотношение Прибыль/Риск:  0.85
#    ⚡ Коэффициент Шарпа (год.): 0.32
# ------------------------------------------------------------
# 🔄 Всего сделок: 621
# ✅ Процент прибыльных сделок: 52.5%
# ✅ График сохранен в 'results_v13.8.png'