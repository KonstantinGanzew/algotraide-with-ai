...
class MTFDataLoader:
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            msg = f"Данные подготовлены. State: {state_features.shape}"
            logger.info(msg)
            print(f"✅ {msg}")
...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V14.0 - УПРОЩЕНИЕ: ТОЛЬКО STATE (MLP)
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. ПОДХОД: Полный отказ от CNN и анализа "картинки" из свечей.
       2. МОДЕЛЬ: Используется стандартная MlpPolicy.
       3. ДАННЫЕ: Агент видит только вектор числовых признаков (RSI, тренды, волатильность и т.д.).
       4. ПАРАМЕТРЫ: Используется сбалансированная конфигурация как отправная точка.
    """
...
class TradingEnv(gym.Env):
    def __init__(self, prices_df: pd.DataFrame, state_features: pd.DataFrame):
        ...
        # НОВОЕ: Простое пространство наблюдений (только state)
        self.state_shape=(3 + self.state_features.shape[1],)
        self.observation_space = spaces.Box(low=-5, high=5, shape=self.state_shape, dtype=np.float32)
    ...
    def _reset_state(self):
        ...
        self.trade_durations, self.close_reasons = [], Counter()
    ...
    def _get_observation(self) -> np.ndarray:
        # УПРОЩЕНО: Больше нет формирования "картинки"
        if self.current_step >= len(self.state_features):
            self.current_step = len(self.state_features) - 1
            
        pos_type = np.sign(self.position_amount)
        pnl_norm, duration_norm = 0.0, 0.0
    ...
    def step(self, action: int):
        try:
            ...
            done = self.current_step >= len(self.state_features) - 1 or self.equity <= 0
    ...
    def _close_position(self, price: float, reason: str) -> float:
        ...
        self.position_amount, self.entry_price, self.last_unrealized_pnl = 0.0, 0.0, 0.0
        return pnl
...
class EarlyStoppingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.model.n_steps != 2048:
             effective_eval_freq = int(self.eval_freq * (self.model.n_steps / 2048))
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V14.0 (State-Only MLP) " + "="*20)
        ...
        # НОВОЕ: Используем MlpPolicy и простую архитектуру сети
        policy_kwargs = dict(net_arch=dict(pi=[256, 128], vf=[256, 128]))
        model = PPO('MlpPolicy', train_env, policy_kwargs=policy_kwargs, learning_rate=TrendTraderConfig.LEARNING_RATE, ent_coef=TrendTraderConfig.ENTROPY_COEF, n_steps=TrendTraderConfig.N_STEPS, batch_size=128, gamma=TrendTraderConfig.GAMMA, verbose=0, device=device)
        
        early_stopping_callback = EarlyStoppingCallback(eval_env=val_env, eval_freq=5000, patience=5, model_save_path='models/')
        logger.info("Модель PPO (MLP) создана. Начало обучения...")
        print("\n🎓 ЭТАП 4: ОПТИМАЛЬНОЕ ОБУЧЕНИЕ (MLP)..."); 
        ...
        # При загрузке модели среда train_env не обязательна, если вы не продолжаете обучение
