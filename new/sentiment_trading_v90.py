...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.15 - АНТИ-FLIP & БОЛЕЕ ДАЛЬНИЙ TP
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. TRANSACTION_PENALTY: Увеличен до 0.02
       2. FLIP_PENALTY: Новый штраф за Flip (смену направления) — 0.02
       3. ATR_TP_MULTIPLIER: Увеличен до 6.0 (более дальний тейк-профит)
       4. ORDER_SIZE_RATIO: Уменьшен до 0.02 (меньше комиссия)
    """
    INITIAL_BALANCE = 10000; TRANSACTION_FEE = 0.001; WINDOW_SIZE = 64; ORDER_SIZE_RATIO = 0.02
    ATR_SL_MULTIPLIER = 3.0; ATR_TP_MULTIPLIER = 6.0; TOTAL_TIMESTEPS = 1000000
...
class TradingEnv(gym.Env):
    ...
    def step(self, action: int):
        try:
            realized_reward, transaction_penalty, hold_reward, flip_penalty = 0.0, 0.0, 0.0, 0.0
            ...
            if action == 0:
                if current_pos != 0: hold_reward = self.cfg.HOLD_REWARD
            elif action == 1:
                if current_pos != 1:
                    transaction_penalty = self.cfg.TRANSACTION_PENALTY
                    if current_pos == -1:
                        flip_penalty = self.cfg.FLIP_PENALTY
            ...
            reward = (realized_reward / self.cfg.INITIAL_BALANCE) + pnl_shaping_reward - transaction_penalty - flip_penalty + hold_reward
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.15 (Анти-Flip & TP6) " + "="*20)
        ...
        # Проверка, была ли сохранена хоть одна модель
