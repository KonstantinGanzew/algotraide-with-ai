...
import logging # ✅ НОВИНКА: Импортируем модуль логирования

# ✅ НОВИНКА: Настройка логирования
logging.basicConfig(
    level=logging.INFO, # Уровень логирования (INFO, DEBUG, WARNING, ERROR, CRITICAL)
    format='%(asctime)s - %(levelname)s - %(message)s',
    filename='trading_v13.log', # Имя файла
    filemode='w' # 'w' - перезаписывать файл при каждом запуске, 'a' - дописывать
)
logger = logging.getLogger(__name__)

# Функции и классы до TradingEnv без изменений
...
"""
🚀 ТОРГОВАЯ СИСТЕМА V13.6 - РЕЖИМ ОТЛАДКИ С ЛОГИРОВАНИЕМ
✅ ЦЕЛЬ: Установить причину "молчаливого провала" путем внедрения подробного логирования.
✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
   1. ЛОГИРОВАНИЕ: Весь процесс, от загрузки данных до каждого шага среды, теперь записывается в файл `trading_v13.log`.
   2. ЗАЩИТА ОТ ОШИБОК: Добавлены блоки `try...except` в критически важные участки кода для отлова и логирования потенциальных сбоев.
✅ ОЖИДАЕМЫЙ РЕЗУЛЬТАТ: Получить лог-файл, который точно покажет, на каком этапе и почему происходит сбой или зависание.
"""
...
class TradingEnv(gym.Env):
    ...
    def _reset_state(self):
        ...
        self.total_fees = 0.0; self.gross_profit = 0.0; self.gross_loss = 0.0; self.trade_durations = []
    ...
    # Методы _get_observation, _get_current_price, _get_current_atr без изменений, т.к. они вспомогательные
    def _get_observation(self)->Dict[str,np.ndarray]:
        ...
        # Проверка на NaN
        if np.isnan(state_obs).any() or np.isnan(image_obs).any():
            logger.error(f"Обнаружен NaN в наблюдении на шаге {self.current_step}. State: {state_obs}, Image has NaN: {np.isnan(image_obs).any()}")
    ...
    def step(self, action: int):
        try: # ✅ Оборачиваем весь шаг в try...except
            realized_reward = 0.0; transaction_penalty = 0.0; 
            action_map = {0: "Hold", 1: "Long", 2: "Short", 3: "Close"}
            
            # Логируем состояние ДО действия
            if self.current_step % 1000 == 0: # Логируем не каждый шаг, чтобы не засорять файл
                 logger.debug(f"Шаг {self.current_step}: Позиция={np.sign(self.position_amount)}, Equity={self.equity:.2f}, Действие агента={action_map.get(action, 'N/A')}")

            # ... остальная логика step ...
            if self.position_amount != 0:
            ...
            if action == 1:
                ...
                if current_pos != 0: transaction_penalty = self.cfg.TRANSACTION_PENALTY; realized_reward += self._close_position(price, "Close by Agent")
            ...
            # Возвращаем "безопасное" состояние, чтобы не прерывать обучение полностью
    ...
    def _close_position(self, price, reason: str):
        duration = self.current_step - self.entry_step; self.trade_durations.append(duration)
...
def main():
    try: # ✅ Оборачиваем весь main в try...except
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.6 " + "="*20)
        # ... остальной код main без изменений, он уже хороший ...
