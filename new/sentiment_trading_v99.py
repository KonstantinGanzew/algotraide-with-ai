...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V14.1 - ФИНАЛЬНЫЙ ТЕСТ: ДОБАВЛЕНИЕ ИМПУЛЬСА
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       - В state добавлен индикатор Momentum для оценки силы текущего движения.
       - Это последняя попытка улучшить результат за счет инжиниринга признаков.
    """
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V14.1 (State-Only + Momentum) " + "="*20)
        ...
        print("\n🎓 ЭТАП 4: ОПТИМАЛЬНОЕ ОБУЧЕНИЕ (MLP + Momentum)..."); 
