...
logging.basicConfig(
    ...
    filename='trading_v13_12.log', # ✅ ИЗМЕНЕНИЕ: Новый файл лога
    filemode='w'
)
...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.12 - ИСПРАВЛЕНИЕ EARLY STOPPING
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. TOTAL_TIMESTEPS: Снижен до 1,000,000 как защитный лимит.
       2. СТРАТЕГИЯ ОБУЧЕНИЯ: Внедрен механизм Early Stopping для поиска оптимальной
          длительности обучения и предотвращения переобучения.
       3. РАЗДЕЛЕНИЕ ДАННЫХ: Теперь 70% на обучение, 10% на валидацию и 20% на тест.
    """
    INITIAL_BALANCE = 10000; TRANSACTION_FEE = 0.001; WINDOW_SIZE = 64; ORDER_SIZE_RATIO = 0.05
    ATR_SL_MULTIPLIER = 2.0; ATR_TP_MULTIPLIER = 4.0; TOTAL_TIMESTEPS = 1000000 # ✅ ИЗМЕНЕНИЕ: Снижено как "потолок"
    LEARNING_RATE = 1e-4; ENTROPY_COEF = 0.001; N_STEPS = 2048; GAMMA = 0.99
    MAX_TRADE_DURATION = 288; TRANSACTION_PENALTY = 0.002; PNL_SHAPING_COEF = 0.02
...
class TradingEnv(gym.Env):
    ...
    def step(self, action: int):
        realized_reward, transaction_penalty = 0.0, 0.0
        if self.position_amount != 0:
...
# ✅ НОВЫЙ КЛАСС: Коллбэк для ранней остановки
class EarlyStoppingCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.eval_freq == 0:
            ...
            if self.verbose > 0:
                print(f"\n---")
                print(f"🧠 EarlyStopping Check @ Step {self.n_calls}:")
                print(f"   - Validation Equity: ${metric:,.2f}")
                print(f"   - Best Equity So Far: ${self.best_metric:,.2f}")

            if metric > self.best_metric:
                self.best_metric = metric
                self.patience_counter = 0
                self.best_model_path = os.path.join(self.model_save_path, f"best_model_step_{self.n_calls}.zip")
    ...
    def _evaluate_performance(self) -> float:
        # Используем обертку DummyVecEnv для совместимости
        eval_vec_env = DummyVecEnv([lambda: self.eval_env])
        obs = eval_vec_env.reset()
        done = False
        while not done:
            action, _ = self.model.predict(obs, deterministic=True)
            # ✅ ИСПРАВЛЕНИЕ: DummyVecEnv.step возвращает 4 значения, а не 5
            obs, _, terminated, truncated = eval_vec_env.step(action)
            done = terminated[0] or truncated[0]
        # Получаем итоговый капитал из среды
        final_equity = eval_vec_env.get_attr('equity')[0]
        return final_equity

def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.12 (Исправление Early Stopping) " + "="*20)
        ...
        # ✅ ИЗМЕНЕНИЕ: Разделение данных на train, validation, test
        train_end_idx = int(len(prices_df) * 0.7)
        val_end_idx = int(len(prices_df) * 0.8)
        ...
        val_env = TradingEnv(val_prices, val_image, val_state) # Не оборачиваем для коллбэка
        ...
        # ✅ ИЗМЕНЕНИЕ: Настройка и использование коллбэков
        ...
        with tqdm(total=TrendTraderConfig.TOTAL_TIMESTEPS, desc="Обучение", unit=" шагов") as pbar:
            ...
            model.learn(total_timesteps=TrendTraderConfig.TOTAL_TIMESTEPS, callback=[tqdm_callback, early_stopping_callback], reset_num_timesteps=False)
        
        logger.info(f"Обучение завершено. Лучшая модель сохранена в: {early_stopping_callback.best_model_path}")
        print(f"\n✅ Обучение остановлено. Лучшая модель: {early_stopping_callback.best_model_path}")

        print("\n💰 ЭТАП 5: ТЕСТИРОВАНИЕ ЛУЧШЕЙ МОДЕЛИ...")
        # ✅ ИЗМЕНЕНИЕ: Загрузка лучшей модели для тестирования
        model = PPO.load(early_stopping_callback.best_model_path, env=train_env)
