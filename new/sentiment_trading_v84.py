...
logging.basicConfig(
    ...
    filename='trading_v13_10.log', # ✅ ИЗМЕНЕНИЕ: Новый файл лога
    filemode='w'
)
...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.10 - БОРЬБА С OVER-TRADING
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. TRANSACTION_PENALTY: Увеличен до 0.002, чтобы штрафовать за "нервные" сделки.
       2. PNL_SHAPING_COEF: Снижен до 0.02, чтобы агент меньше реагировал на рыночный шум.
       3. REPORTING: Добавлена детальная статистика по причинам закрытия сделок.
    """
    ...
    TRANSACTION_PENALTY = 0.002   # ✅ ИЗМЕНЕНИЕ: Увеличиваем штраф
    PNL_SHAPING_COEF = 0.02     # ✅ ИЗМЕНЕНИЕ: Снижаем чувствительность к PnL
...
class TradingEnv(gym.Env):
    ...
    def _reset_state(self):
        ...
        self.close_reasons = Counter() # ✅ ИЗМЕНЕНИЕ: Сбор статистики по закрытиям
    ...
    def step(self, action: int):
        try:
            ...
            # Проверяем, не была ли позиция уже закрыта на этом шаге (например, по SL)
            if self.position_amount != 0:
            ...
            # Если позиции нет, а агент хочет открыть
            elif action == 1 and current_pos == 0:
                transaction_penalty = self.cfg.TRANSACTION_PENALTY
                self._open_position(price, is_long=True)
    ...
    def _close_position(self, price: float, reason: str) -> float:
        # ✅ ИЗМЕНЕНИЕ: Сохраняем причину закрытия
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.10 (Борьба с over-trading) " + "="*20)
        ...
        # ✅ ИЗМЕНЕНИЕ: Детальная статистика по закрытиям
