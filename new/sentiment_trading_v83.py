...
logging.basicConfig(
    ...
    filename='trading_v13_9.log', # ✅ ИЗМЕНЕНИЕ: Новый файл лога
    filemode='w'
)
...
class CustomCombinedExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space: spaces.Dict, features_dim: int = 256):
        ...
        # ✅ ИСПРАВЛЕНИЕ: Использование .spaces для доступа к элементам Dict-пространства
...
class MTFDataLoader:
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            # ✅ ИСПРАВЛЕНИЕ: Явное указание 'by' в sort_values
            merged_df=dfs['5m']; [merged_df:=pd.merge_asof(merged_df.sort_values(by='timestamp'),dfs[tf][['timestamp',f'trend_{tf}',f'rsi_{tf}',f'atr_{tf}']].sort_values(by='timestamp'),on='timestamp',direction='backward') for tf in ['1h','4h','1d']]
            ...
            # ✅ ИСПРАВЛЕНИЕ: Замена inplace=True на переприсваивание
...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.9 - РЕЖЕМ УБЫТКИ, ТЯНЕМ ПРИБЫЛЬ
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. MAX_TRADE_DURATION: Теперь принудительно закрывает сделку по истечении времени.
       2. REWARD_SYSTEM: Внедрен симметричный штраф/бонус за удержание убыточной/прибыльной позиции.
    """
    ...
    N_STEPS = 2048
    GAMMA = 0.99
    MAX_TRADE_DURATION = 288 # 288 * 5 min = 24 hours
    TRANSACTION_PENALTY = 0.001
    PNL_SHAPING_COEF = 0.05 # Коэффициент для бонуса/штрафа за удержание
...
class TradingEnv(gym.Env):
    ...
    def _reset_state(self):
        self.balance, self.equity = self.cfg.INITIAL_BALANCE, self.cfg.INITIAL_BALANCE; self.current_step = self.cfg.WINDOW_SIZE; self.position_amount = 0.0; self.entry_price = 0.0; self.entry_step = 0; self.stop_loss_price = 0.0; self.take_profit_price = 0.0; self.trades = []; self.total_fees = 0.0; self.gross_profit = 0.0; self.gross_loss = 0.0; self.trade_durations = []

    def reset(self, seed=None, options=None):
        logger.info(f"Сброс среды. Начальный шаг: {self.cfg.WINDOW_SIZE}"); super().reset(seed=seed); self._reset_state(); return self._get_observation(), {}
    ...
    def _get_observation(self) -> Dict[str, np.ndarray]:
        ...
        if np.isnan(state_obs).any() or np.isnan(image_obs).any(): logger.error(f"Обнаружен NaN в наблюдении на шаге {self.current_step}.")
    ...
    def step(self, action: int):
        try:
            ...
            if self.position_amount != 0:
                ...
                # ✅ ИЗМЕНЕНИЕ V13.9: Принудительное закрытие по истечению времени
                elif (self.current_step - self.entry_step) >= self.cfg.MAX_TRADE_DURATION:
                    realized_reward = self._close_position(self._get_current_price(), "Max Duration")
            ...
            if action == 1 and current_pos != 1: 
                ...
                if current_pos == -1: realized_reward += self._close_position(price, "Flip to Long")
                self._open_position(price, is_long=True)
            elif action == 2 and current_pos != -1:
                transaction_penalty = self.cfg.TRANSACTION_PENALTY
                if current_pos == 1: realized_reward += self._close_position(price, "Flip to Short")
            ...
            # ✅ ИЗМЕНЕНИЕ V13.9: СИММЕТРИЧНЫЙ ШТРАФ/БОНУС
            # Штрафуем за удержание убыточной позиции и вознаграждаем за удержание прибыльной.
            # Это мотивирует резать убытки и давать прибыли расти.
            pnl_shaping_reward = 0.0
            if self.position_amount != 0:
                pnl_shaping_reward = (unrealized_pnl / self.cfg.INITIAL_BALANCE) * self.cfg.PNL_SHAPING_COEF

            reward = (realized_reward / self.cfg.INITIAL_BALANCE) + pnl_shaping_reward - transaction_penalty
            ...
            if done and self.position_amount != 0:
                final_pnl = self._close_position(self._get_current_price(), "End of Episode"); reward += final_pnl / self.cfg.INITIAL_BALANCE
    ...
    def _open_position(self, price: float, is_long: bool):
        ...
        if self.balance > order_size and order_size > 0:
            ...
            logger.info(f"Шаг {self.current_step}: ОТКРЫТА {'LONG' if is_long else 'SHORT'} позиция. Цена: {price:.2f}")
    ...
    def _close_position(self, price: float, reason: str) -> float:
        ...
        logger.info(f"Шаг {self.current_step}: ЗАКРЫТА позиция. Причина: {reason}. PnL: {pnl:.2f}, Длительность: {duration} шагов")
        self.position_amount, self.entry_price = 0.0, 0.0
        return pnl

def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.9 (Режем убытки, тянем прибыль) " + "="*20)
        ...
        print("\n🎓 ЭТАП 4: ОБУЧЕНИЕ АГЕНТА ДИСЦИПЛИНЕ..."); 
