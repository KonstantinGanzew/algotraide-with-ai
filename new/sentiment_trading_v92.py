...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.17 - ГИБРИДНОЕ ВОЗНАГРАЖДЕНИЕ
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. REWARD SYSTEM: Гибридная модель. Основной сигнал - PnL, но с добавлением
          небольшого штрафа за транзакцию и бонуса за удержание позиции.
       2. TRANSACTION_COST_PENALTY: Новый небольшой штраф (0.002) за открытие/переворот.
       3. HOLD_REWARD_BONUS: Новый микро-бонус (0.00001) за удержание позиции.
       4. ENTROPY_COEF: Снижен до 0.005, чтобы уменьшить хаотичность действий.
    """
    ...
    LEARNING_RATE = 1e-4; ENTROPY_COEF = 0.005; N_STEPS = 2048; GAMMA = 0.99
    MAX_TRADE_DURATION = 288 

    # Новые гибридные параметры вознаграждения
    TRANSACTION_COST_PENALTY = 0.002   # Штраф, представляющий комиссию/спред
    HOLD_REWARD_BONUS = 0.00001        # Небольшой стимул держать позицию
...
class TradingEnv(gym.Env):
    ...
    def step(self, action: int):
        try:
            ...
            # 1. Проверяем принудительное закрытие позиции
            if self.position_amount != 0:
            ...
            # 2. Обрабатываем действие агента и назначаем искусственные награды/штрафы
            current_pos_sign = np.sign(self.position_amount)
            price = self._get_current_price()
            if action == 0:  # Держать
                if current_pos_sign != 0:
                    hold_reward = self.cfg.HOLD_REWARD_BONUS
            elif action == 1 and current_pos_sign != 1:  # Открыть/перевернуться в лонг
            ...
            # 3. Рассчитываем основное вознаграждение на основе изменения PnL
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.17 (Гибридное вознаграждение) " + "="*20)
