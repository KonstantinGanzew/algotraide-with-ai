...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.19 - ФИЛЬТР ШУМА И МАКСИМАЛЬНАЯ ИЗБИРАТЕЛЬНОСТЬ
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. ORDER_SIZE_RATIO: Возвращаем к безопасным 0.02 (2%). Это основа.
       2. TRANSACTION_COST_PENALTY: Значительно увеличен до 0.005. Каждая сделка
          должна быть оправдана. Цель - резкое снижение числа сделок.
       3. HOLD_REWARD_BONUS: Значительно увеличен до 0.00005. Это мощный стимул
          не закрывать сделку раньше времени и фильтровать рыночный шум.
       4. ATR_TP_MULTIPLIER: Увеличен до 6.0. Если уж входить в сделку, то
          с высоким потенциалом прибыли.
    """
    ...
    # Гибридные параметры вознаграждения
    TRANSACTION_COST_PENALTY = 0.005
    HOLD_REWARD_BONUS = 0.00005
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.19 (Фильтр шума) " + "="*20)
