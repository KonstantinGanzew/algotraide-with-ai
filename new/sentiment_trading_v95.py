...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.21 - ОСТОРОЖНОЕ РАСШИРЕНИЕ
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       - Основано на успешной V13.19.
       - TRANSACTION_COST_PENALTY: Слегка снижен с 0.005 до 0.004, чтобы
         поощрить агента совершить немного больше качественных сделок,
         сохраняя при этом высокую избирательность.
    """
    ...
    TRANSACTION_COST_PENALTY = 0.004  # <-- ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.21 (Осторожное расширение) " + "="*20)
