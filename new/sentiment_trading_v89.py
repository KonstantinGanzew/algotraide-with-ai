...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.14 - УЛУЧШЕНИЕ ТОЧНОСТИ ВХОДА
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. ATR_SL_MULTIPLIER: Увеличен с 2.0 до 3.0. Мы даем сделке больше
          "пространства для маневра", чтобы избежать закрытия по стоп-лоссу
          из-за рыночного шума. Цель - повысить Win Rate.
    """
    INITIAL_BALANCE = 10000; TRANSACTION_FEE = 0.001; WINDOW_SIZE = 64; ORDER_SIZE_RATIO = 0.05
    ATR_SL_MULTIPLIER = 3.0; ATR_TP_MULTIPLIER = 4.0; TOTAL_TIMESTEPS = 1000000 # ✅ ИЗМЕНЕНИЕ: Более широкий SL
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.14 (Широкий SL) " + "="*20)
