...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V13.16 - PNL-BASED REWARD & EXPLORATION FOCUS
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       1. REWARD SYSTEM: Полностью переработана. Вознаграждение теперь = изменение PnL за шаг.
       2. ENTROPY_COEF: Увеличен до 0.01 для стимуляции исследования.
       3. ATR_TP_MULTIPLIER: Уменьшен до 4.5 (R/R = 1.5) для более частого получения наград.
       4. Штрафы и искусственные награды убраны.
    """
    INITIAL_BALANCE = 10000; TRANSACTION_FEE = 0.001; WINDOW_SIZE = 64; ORDER_SIZE_RATIO = 0.02
    ATR_SL_MULTIPLIER = 3.0; ATR_TP_MULTIPLIER = 4.5; TOTAL_TIMESTEPS = 1000000
    LEARNING_RATE = 3e-4; ENTROPY_COEF = 0.01; N_STEPS = 2048; GAMMA = 0.99
    MAX_TRADE_DURATION = 288 # 288 * 5-min = 1 day
...
class TradingEnv(gym.Env):
    ...
    def _reset_state(self):
        ...
        self.last_unrealized_pnl = 0.0 # НОВОЕ: Для расчета PnL-based reward
    ...
    def step(self, action: int):
        try:
            ...
            # 1. Проверяем принудительное закрытие позиции (SL/TP/Duration)
            if self.position_amount != 0:
            ...
            # 3. Рассчитываем вознаграждение на основе изменения PnL
            ...
            # Вознаграждение = реализованная прибыль/убыток + изменение нереализованной прибыли/убытка
            reward = (realized_pnl + pnl_change) / self.cfg.INITIAL_BALANCE
            
            # 4. Обновляем состояние среды
    ...
    def _open_position(self, price: float, is_long: bool):
        ...
        if self.balance > order_size and order_size > 0:
            ...
            self.last_unrealized_pnl = 0.0 # Сбрасываем при открытии новой позиции
    ...
    def _close_position(self, price: float, reason: str) -> float:
        ...
        self.last_unrealized_pnl = 0.0 # Сбрасываем PnL при закрытии
        return pnl
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V13.16 (PNL Reward & Exploration) " + "="*20)
