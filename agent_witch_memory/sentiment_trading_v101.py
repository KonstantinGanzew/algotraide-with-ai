...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V15.1 - СТИМУЛЯЦИЯ АКТИВНОСТИ LSTM-АГЕНТА
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       - Основано на обучаемой модели V15.0.
       - TRANSACTION_COST_PENALTY: Снижен до 0.0015, чтобы уменьшить "страх входа".
       - ENTROPY_COEF: Увеличен до 0.01, чтобы поощрить исследование новых,
         возможно, более частых стратегий.
    """
    ...
    TOTAL_TIMESTEPS = 1000000; LEARNING_RATE = 1e-4
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V15.1 (Стимуляция LSTM) " + "="*20)
