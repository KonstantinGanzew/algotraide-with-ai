# ===================================================================
# ЭТАП 1: ИМПОРТ ВСЕХ НЕОБХОДИМЫХ БИБЛИОТЕК
# ===================================================================
import numpy as np
import pandas as pd
import torch
...
# ===================================================================
# ЭТАП 2: ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ И КЛАССЫ КОНФИГУРАЦИИ
# ===================================================================
...
class TrendTraderConfig:
    ...
    ORDER_SIZE_RATIO = 0.02; ATR_SL_MULTIPLIER = 3.0; ATR_TP_MULTIPLIER = 5.5
    TOTAL_TIMESTEPS = 1000000; LEARNING_RATE = 1e-4; ENTROPY_COEF = 0.01
    N_STEPS = 4096; GAMMA = 0.99; MAX_TRADE_DURATION = 288
    TRANSACTION_COST_PENALTY = 0.0015; HOLD_REWARD_BONUS = 0.00002
    LSTM_HIDDEN_SIZE = 64; N_LSTM_LAYERS = 1

# ===================================================================
# ЭТАП 3: КЛАССЫ ДЛЯ ДАННЫХ И СРЕДЫ
# ===================================================================
class MTFDataLoader:
    ...
    def _calc_indicators(self, df: pd.DataFrame, suffix: str) -> pd.DataFrame:
        try:
            ...
            rs = gain / loss; df[f'rsi_{suffix}'] = 100 - (100 / (1 + rs))
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            ...
            initial_rows = len(merged_df); merged_df.replace([np.inf, -np.inf], np.nan, inplace=True)
            ...
            merged_df['momentum_12'] = (merged_df['close'] - merged_df['close'].shift(12)) / merged_df['close'].shift(12)
            merged_df.dropna(inplace=True); logger.info(f"Удалено {initial_rows - len(merged_df)} строк с NaN/inf значениями.")
            ...
            state_features['momentum_norm'] = (merged_df['momentum_12'] - merged_df['momentum_12'].mean()) / merged_df['momentum_12'].std()
            for tf in ['1h', '4h', '1d']:
                state_features[f'trend_{tf}'] = merged_df[f'trend_{tf}']; state_features[f'rsi_{tf}_norm'] = (merged_df[f'rsi_{tf}'] - 50) / 50
            ...
            min_len = min(len(prices_df), len(state_features))
            prices_df = prices_df.iloc[-min_len:].reset_index(drop=True); state_features = state_features.iloc[-min_len:].reset_index(drop=True)
            msg = f"Данные подготовлены. State: {state_features.shape}"; logger.info(msg); print(f"✅ {msg}")
            return prices_df, state_features
        except Exception as e: logger.critical(f"Критическая ошибка при загрузке данных: {e}", exc_info=True); raise

class TradingEnv(gym.Env):
    def __init__(self, prices_df: pd.DataFrame, state_features: pd.DataFrame):
        super().__init__(); self.prices_df = prices_df; self.state_features = state_features; self.cfg = TrendTraderConfig()
        self.action_space = spaces.Discrete(3); self.state_shape=(3 + self.state_features.shape[1],); self.observation_space = spaces.Box(low=-5, high=5, shape=self.state_shape, dtype=np.float32)
        self._reset_state()
    def _reset_state(self):
        self.balance, self.equity = self.cfg.INITIAL_BALANCE, self.cfg.INITIAL_BALANCE; self.current_step = self.cfg.WINDOW_SIZE
        self.position_amount, self.entry_price, self.entry_step = 0.0, 0.0, 0
        self.stop_loss_price, self.take_profit_price = 0.0, 0.0
        self.trades, self.total_fees, self.gross_profit, self.gross_loss = [], 0.0, 0.0, 0.0
        self.trade_durations, self.close_reasons = [], Counter(); self.last_unrealized_pnl = 0.0
    def reset(self, seed=None, options=None):
        super().reset(seed=seed); self._reset_state(); return self._get_observation(), {}
    def _get_observation(self) -> np.ndarray:
        if self.current_step >= len(self.state_features): self.current_step = len(self.state_features) - 1
        pos_type = np.sign(self.position_amount); pnl_norm, duration_norm = 0.0, 0.0
        ...
        operational_state = np.array([pos_type, pnl_norm, duration_norm]); analytical_state = self.state_features.iloc[self.current_step].values
        return np.concatenate([operational_state, analytical_state]).astype(np.float32)
    ...
    def step(self, action: int):
        try:
            realized_pnl, transaction_penalty, hold_reward = 0.0, 0.0, 0.0
            if self.position_amount != 0:
                low, high = self.prices_df.iloc[self.current_step][['low', 'high']]
                is_long = self.position_amount > 0
                if (is_long and low <= self.stop_loss_price) or (not is_long and high >= self.stop_loss_price): realized_pnl = self._close_position(self.stop_loss_price, "SL")
                elif (is_long and high >= self.take_profit_price) or (not is_long and low <= self.take_profit_price): realized_pnl = self._close_position(self.take_profit_price, "TP")
                elif (self.current_step - self.entry_step) >= self.cfg.MAX_TRADE_DURATION: realized_pnl = self._close_position(self._get_current_price(), "Max Duration")
            current_pos_sign = np.sign(self.position_amount); price = self._get_current_price()
            if action == 0:
                if current_pos_sign != 0: hold_reward = self.cfg.HOLD_REWARD_BONUS
            elif action == 1 and current_pos_sign != 1:
                transaction_penalty = self.cfg.TRANSACTION_COST_PENALTY
                if current_pos_sign == -1: realized_pnl += self._close_position(price, "Flip to Long")
                self._open_position(price, is_long=True)
            elif action == 2 and current_pos_sign != -1:
                transaction_penalty = self.cfg.TRANSACTION_COST_PENALTY
                if current_pos_sign == 1: realized_pnl += self._close_position(price, "Flip to Short")
            ...
            pnl_change = current_unrealized_pnl - self.last_unrealized_pnl; self.last_unrealized_pnl = current_unrealized_pnl
            pnl_based_reward = (realized_pnl + pnl_change) / self.cfg.INITIAL_BALANCE
            reward = pnl_based_reward + hold_reward - transaction_penalty
            self.equity = self.balance + current_unrealized_pnl; done = self.current_step >= len(self.state_features) - 1 or self.equity <= 0
            if done and self.position_amount != 0:
                final_pnl = self._close_position(self._get_current_price(), "End of Episode"); reward += final_pnl / self.cfg.INITIAL_BALANCE; self.equity = self.balance
            return self._get_observation(), reward, done, False, {'equity': self.equity}
        except Exception as e: logger.critical(f"Критическая ошибка на шаге {self.current_step}: {e}", exc_info=True); return self._get_observation(), 0, True, False, {'equity': self.equity}
    def _open_position(self, price: float, is_long: bool):
        ...
        if self.balance > order_size and order_size > 0:
            fee = order_size * self.cfg.TRANSACTION_FEE; self.total_fees += fee; self.balance -= order_size; self.position_amount = (order_size - fee) / price * (1 if is_long else -1); self.entry_price = price; self.last_unrealized_pnl = 0.0
    def _close_position(self, price: float, reason: str) -> float:
        self.close_reasons[reason] += 1; duration = self.current_step - self.entry_step; self.trade_durations.append(duration); size, is_long = abs(self.position_amount), self.position_amount > 0
        ...
        self.balance += entry_value + pnl; self.trades.append(pnl); self.position_amount, self.entry_price, self.last_unrealized_pnl = 0.0, 0.0, 0.0
        return pnl

# ===================================================================
# ЭТАП 4: КАСТОМНАЯ ПОЛИТИКА И КОЛЛБЭКИ
# ===================================================================
...
class CustomRecurrentPolicy(RecurrentActorCriticPolicy):
    ...
    def _build(self, lr_schedule: Any) -> None:
        ...
        self.action_net = nn.Linear(latent_dim, self.action_space.n)
        self.value_net = nn.Linear(latent_dim, 1)

    def forward(
        self, obs: torch.Tensor, lstm_states: Tuple, episode_starts: torch.Tensor, deterministic: bool = False
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Tuple]:
        obs = obs.unsqueeze(0)
        pi_states = lstm_states.pi
        lstm_output, new_pi_states = self.lstm(obs, pi_states)
        lstm_output = lstm_output.squeeze(0)
        latent_pi = lstm_output
        latent_vf = lstm_output
        distribution = self._get_action_dist_from_latent(latent_pi)
        values = self.value_net(latent_vf)
        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        new_lstm_states = type(lstm_states)(pi=new_pi_states, vf=new_pi_states)
...
class EarlyStoppingCallback(BaseCallback):
    def __init__(self, eval_env: gym.Env, eval_freq: int, patience: int, model_save_path: str, verbose: int = 1):
        super().__init__(verbose); self.eval_env = eval_env; self.eval_freq = eval_freq; self.patience = patience; self.model_save_path = model_save_path
        self.best_metric = -np.inf; self.patience_counter = 0; self.best_model_path = ""
        os.makedirs(self.model_save_path, exist_ok=True)
    def _on_step(self) -> bool:
        effective_eval_freq = int(self.eval_freq * (self.model.n_steps / 2048)) if self.model.n_steps != 2048 else self.eval_freq
        if self.n_calls > 0 and self.n_calls % effective_eval_freq == 0:
            ...
            if metric > self.best_metric:
                ...
                logger.info(f"New best model found! Saved to {self.best_model_path}");
            ...
            if self.patience_counter >= self.patience:
                if self.verbose > 0: print(f"\nStopping training early: patience reached."); logger.warning(f"Stopping early at step {self.n_calls}."); return False
            if self.verbose > 0: print(f"---")
        return True
    def _evaluate_performance(self) -> float:
        ...
        final_equity = self.eval_env.get_attr('equity')[0]
        return final_equity


# ===================================================================
# ЭТАП 5: ОСНОВНАЯ ЛОГИКА (main)
# ===================================================================

def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V16.0 (LSTM) " + "="*20)
        ...
        prices_df, state_features = data_loader.load_and_prepare_data()
        ...
        train_env = DummyVecEnv([lambda: TradingEnv(train_prices, train_state)])
        val_env = DummyVecEnv([lambda: TradingEnv(val_prices, val_state)])
        
        policy_kwargs = dict(
            lstm_hidden_size=TrendTraderConfig.LSTM_HIDDEN_SIZE,
            n_lstm_layers=TrendTraderConfig.N_LSTM_LAYERS,
        )

        model = RecurrentPPO(
            CustomRecurrentPolicy, train_env, policy_kwargs=policy_kwargs, learning_rate=TrendTraderConfig.LEARNING_RATE,
            ent_coef=TrendTraderConfig.ENTROPY_COEF, n_steps=TrendTraderConfig.N_STEPS, batch_size=128,
        ...
        logger.info("Модель RecurrentPPO (LSTM) создана. Начало обучения...")
        print("\n🎓 ЭТАП 4: ОБУЧЕНИЕ АГЕНТА (LSTM)..."); 
        with tqdm(total=TrendTraderConfig.TOTAL_TIMESTEPS, desc="Обучение", unit=" шагов") as pbar:
            tqdm_callback = TqdmCallback(pbar)
            model.learn(
                total_timesteps=TrendTraderConfig.TOTAL_TIMESTEPS,
                callback=[tqdm_callback, early_stopping_callback],
                reset_num_timesteps=False
            )
            
        if not early_stopping_callback.best_model_path:
            logger.warning("Обучение завершено без улучшений."); print("\n⚠️ Обучение завершено без улучшений."); model.save("models/last_model.zip"); best_model_path = "models/last_model.zip"
        else:
            best_model_path = early_stopping_callback.best_model_path
            logger.info(f"Обучение завершено. Лучшая модель: {best_model_path}"); print(f"\n✅ Обучение остановлено. Лучшая модель: {best_model_path}")
        
        print("\n💰 ЭТАП 5: ТЕСТИРОВАНИЕ ЛУЧШЕЙ МОДЕЛИ...")
        model = RecurrentPPO.load(best_model_path, device=device)
        
        test_env_raw = TradingEnv(test_prices, test_state)
        test_env = DummyVecEnv([lambda: test_env_raw])
        obs = test_env.reset()
        
        lstm_states = None; episode_starts = np.ones((1,), dtype=bool)
        equity_history, price_history = [test_env_raw.equity], [test_env_raw._get_current_price()]
        test_steps = len(test_prices) - test_env_raw.cfg.WINDOW_SIZE - 1
        ...
        for _ in tqdm(range(test_steps), desc="Тестирование"):
            ...
            episode_starts = np.array([done])
            equity_history.append(infos[0]['equity']); price_history.append(test_env_raw._get_current_price())
            # <<< КОНЕЦ ИСПРАВЛЕНИЯ 2
            if done: break
        ...
        env = test_env_raw
        equity_series = pd.Series(equity_history, index=test_prices.index[env.cfg.WINDOW_SIZE : env.cfg.WINDOW_SIZE + len(equity_history)])
        peak = equity_series.expanding(min_periods=1).max(); drawdown = (equity_series - peak) / peak; max_drawdown = drawdown.min()
        returns = equity_series.pct_change().dropna(); annualization_factor = np.sqrt(252 * 288) # 288 5-min intervals in a day
        profit_factor = abs(env.gross_profit / env.gross_loss) if env.gross_loss != 0 else float('inf')
        sharpe_ratio = (returns.mean() / returns.std()) * annualization_factor if returns.std() > 0 else 0
        avg_duration_steps = np.mean(env.trade_durations) if env.trade_durations else 0; avg_duration_hours = (avg_duration_steps * 5) / 60
        wins = [t for t in env.trades if t > 0]; losses = [t for t in env.trades if t < 0]
        ...
        trades=len(env.trades); win_rate=(len(wins)/trades)*100 if trades > 0 else 0
        ...
        print("📈 Детальная статистика PnL:"); print(f"   🟢 Валовая прибыль: ${env.gross_profit:,.2f}"); print(f"   🔴 Валовый убыток:  ${env.gross_loss:,.2f}"); print(f"   💸 Всего комиссий:  ${env.total_fees:,.2f}"); print(f"   ⚖️ Профит-фактор:    {profit_factor:.2f}"); print("-"*60)
