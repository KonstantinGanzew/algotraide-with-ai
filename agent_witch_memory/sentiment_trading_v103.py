...
# MTFDataLoader и TradingEnv остаются такими же, как в V14.1/V15.1
class MTFDataLoader:
...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V15.3 - МАРАФОН ОБУЧЕНИЯ
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       - Возвращаемся к стабильной и прибыльной конфигурации V15.1.
       - TOTAL_TIMESTEPS: Увеличено до 2,000,000, чтобы дать агенту вдвое больше времени.
       - PATIENCE: Увеличено до 10 в EarlyStoppingCallback для более глубокого поиска.
    """
    INITIAL_BALANCE = 10000; TRANSACTION_FEE = 0.001; WINDOW_SIZE = 64
    ATR_SL_MULTIPLIER = 3.0; ATR_TP_MULTIPLIER = 5.5
    ...
    # Параметры из успешной V15.1
    ORDER_SIZE_RATIO = 0.02
    ENTROPY_COEF = 0.01
    TRANSACTION_COST_PENALTY = 0.0015
...
class EarlyStoppingCallback(BaseCallback):
    ...
    def _evaluate_performance(self) -> float:
        eval_env = self.eval_env; obs, _ = eval_env.reset()
...
def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V15.3 (Марафон обучения) " + "="*20)
        ...
        policy_kwargs = dict(net_arch=dict(pi=[128], vf=[128]), lstm_hidden_size=TrendTraderConfig.LSTM_HIDDEN_SIZE, n_lstm_layers=TrendTraderConfig.N_LSTM_LAYERS)
        ...
        # --- ИЗМЕНЕНИЕ ---
        early_stopping_callback = EarlyStoppingCallback(eval_env=val_env, eval_freq=5000, patience=10, model_save_path='models/')
        # ------------------
        ...
        test_env = TradingEnv(test_prices, test_state); obs, _ = test_env.reset()
        lstm_states = None; done = False
