...
# MTFDataLoader и TradingEnv остаются такими же, как в V14.1
class MTFDataLoader:
...
class TrendTraderConfig:
    """
    🚀 КОНФИГУРАЦИЯ V15.0 - АГЕНТ С ПАМЯТЬЮ (LSTM)
    ✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
       - МОДЕЛЬ: Используется RecurrentPPO с политикой MlpLstmPolicy.
       - АРХИТЕКТУРА: Агент теперь имеет "память" для анализа последовательностей.
       - ПАРАМЕТРЫ: Добавлены настройки для LSTM-слоев.
    """
    ...
    TOTAL_TIMESTEPS = 1000000; LEARNING_RATE = 1e-4; ENTROPY_COEF = 0.005
    N_STEPS = 4096; GAMMA = 0.99; MAX_TRADE_DURATION = 288 
    TRANSACTION_COST_PENALTY = 0.0025; HOLD_REWARD_BONUS = 0.00002

    # НОВЫЕ ПАРАМЕТРЫ ДЛЯ LSTM
    LSTM_HIDDEN_SIZE = 64
    N_LSTM_LAYERS = 1

class TradingEnv(gym.Env): # Этот класс не требует изменений
    def __init__(self, prices_df: pd.DataFrame, state_features: pd.DataFrame):
        super().__init__()
        self.prices_df = prices_df; self.state_features = state_features; self.cfg = TrendTraderConfig()
        self.action_space = spaces.Discrete(3)
        self.state_shape=(3 + self.state_features.shape[1],); self.observation_space = spaces.Box(low=-5, high=5, shape=self.state_shape, dtype=np.float32)
        self._reset_state()
...
class EarlyStoppingCallback(BaseCallback): # Этот класс не требует изменений
    def __init__(self, eval_env: gym.Env, eval_freq: int, patience: int, model_save_path: str, verbose: int = 1):
        super(EarlyStoppingCallback, self).__init__(verbose); self.eval_env = eval_env; self.eval_freq = eval_freq; self.patience = patience; self.model_save_path = model_save_path
    ...
    def _on_step(self) -> bool:
        ...
        if self.n_calls > 0 and self.n_calls % effective_eval_freq == 0:
            ...
            if self.patience_counter >= self.patience:
                ...
                logger.warning(f"Stopping training early at step {self.n_calls}."); return False
            if self.verbose > 0: print(f"---")
        return True
    def _evaluate_performance(self) -> float: # Тестовый цикл для callback тоже меняется
        eval_env = self.eval_env
        obs, _ = eval_env.reset()
        done, lstm_states = False, None
        while not done:
            action, lstm_states = self.model.predict(obs, state=lstm_states, episode_start=np.array([done]), deterministic=True)
            obs, _, terminated, truncated, _ = eval_env.step(action)
            done = terminated or truncated
        return eval_env.equity

def main():
    try:
        logger.info("="*20 + " ЗАПУСК СИСТЕМЫ V15.0 (LSTM) " + "="*20)
        ...
        # Важно: для RecurrentPPO нельзя оборачивать eval_env в DummyVecEnv внутри callback
        val_env = TradingEnv(val_prices, val_state) 

        # НОВОЕ: Настройка политики для LSTM
        policy_kwargs = dict(
            net_arch=dict(pi=[128], vf=[128]),
            lstm_hidden_size=TrendTraderConfig.LSTM_HIDDEN_SIZE,
            n_lstm_layers=TrendTraderConfig.N_LSTM_LAYERS,
        )
        # НОВОЕ: Создание модели RecurrentPPO
        model = RecurrentPPO('MlpLstmPolicy', train_env, policy_kwargs=policy_kwargs, learning_rate=TrendTraderConfig.LEARNING_RATE, ent_coef=TrendTraderConfig.ENTROPY_COEF, n_steps=TrendTraderConfig.N_STEPS, batch_size=128, gamma=TrendTraderConfig.GAMMA, verbose=0, device=device)
        ...
        print("\n🎓 ЭТАП 4: ОБУЧЕНИЕ АГЕНТА С ПАМЯТЬЮ (LSTM)..."); 
        ...
        if not early_stopping_callback.best_model_path:
            ...
            print("\n⚠️ Обучение завершено без улучшений. Тестирование последней модели."); model.save("models/last_model.zip"); best_model_path = "models/last_model.zip"
        else:
            best_model_path = early_stopping_callback.best_model_path
            logger.info(f"Обучение завершено. Лучшая модель сохранена в: {best_model_path}"); print(f"\n✅ Обучение остановлено. Лучшая модель: {best_model_path}")

        print("\n💰 ЭТАП 5: ТЕСТИРОВАНИЕ ЛУЧШЕЙ МОДЕЛИ...")
        model = RecurrentPPO.load(best_model_path, device=device) 
        
        test_env = TradingEnv(test_prices, test_state)
        obs, _ = test_env.reset()
        # НОВОЕ: Управление состоянием LSTM и флагом завершения эпизода
        lstm_states = None
        done = False
        ...
        for _ in tqdm(range(test_steps), desc="Тестирование"):
            ...
            equity_history.append(info['equity']); price_history.append(test_env._get_current_price())
            if done: break
