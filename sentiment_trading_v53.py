...
"""
🚀 ТОРГОВАЯ СИСТЕМА V5.9 - ПУРИСТ
✅ РАДИКАЛЬНОЕ ИЗМЕНЕНИЕ: Убраны ВСЕ промежуточные награды (за удержание и ожидание).
    - Агент получает награду ТОЛЬКО за закрытую сделку (реализованный PnL).
✅ АДАПТАЦИЯ К SPARSE REWARDS:
    - Gamma увеличена до 0.999 для повышения "дальнозоркости" агента.
    - Время обучения увеличено до 1,000,000 шагов для лучшего исследования.
✅ ИСПРАВЛЕНО: Переход на MlpPolicy для корректной работы с временными рядами.
✅ Цель: Заставить агента учиться настоящей торговле, а не "геймингу" системы вознаграждений. Создать робастную базовую стратегию.
"""
...
class TrendTraderConfig:
    ...
    WINDOW_SIZE = 64
    TOTAL_TIMESTEPS = 1000000 # Увеличиваем
    LEARNING_RATE = 3e-4
    ENTROPY_COEF = 0.01
    GAMMA = 0.999 # Увеличиваем
    TREND_PROFIT_BONUS = 0.1
    TRADE_PENALTY = 0.01 # Штраф оставляем, но он теперь применяется к reward'у за сделку
...
class TradingEnv(gym.Env):
    ...
    # ИЗМЕНЕНО: Радикально упрощенная функция step
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        current_price = self._get_current_price()
        reward = 0.0 # Награда по умолчанию равна нулю
        done = False

        if action == 1 and self.position_amount == 0:
            self._open_position(current_price)
            # Штраф теперь будет вычтен из награды за закрытие сделки
    ...
    def _close_position(self, price: float) -> float:
        ...
        # Вычитаем штраф за торговлю из итоговой награды за сделку
...
def main():
    print("🚀 СИСТЕМА V5.9 (Пурист с MLP) - ЗАПУСК")
    ...
    # Используем MLP: создаем векторизованное окружение для Sparse Rewards
    env_fn = lambda: TradingEnv(train_features, train_prices)
    vec_env = DummyVecEnv([lambda: TradingEnv(train_features, train_prices)])

    # Архитектура MLP для Sparse Rewards (глубже для лучшего обучения на редких наградах)
    policy_kwargs = dict(
        net_arch=dict(pi=[256, 128, 64], vf=[256, 128, 64])  # Глубокая сеть для sparse rewards
    )
    ...
    print("\n🎓 ЭТАП 4: ОБУЧЕНИЕ ПУРИСТА (ТОЛЬКО Sparse Rewards за закрытые сделки)...")
    model.learn(total_timesteps=TrendTraderConfig.TOTAL_TIMESTEPS)
    
    print("\n💰 ЭТАП 5: ТЕСТИРОВАНИЕ НА НЕВИДИМЫХ ДАННЫХ...")
    # Создаем тестовое окружение с философией Пуриста
    ...
    # Анализируем результаты философии "Пуриста"
    print("=" * 60)
    ...
    print(f"🎯 Философия: ТОЛЬКО Sparse Rewards (награды за закрытые сделки), Gamma={TrendTraderConfig.GAMMA}")
    
    plt.figure(figsize=(15, 7))
    plt.title('Результаты на тестовой выборке (V5.9 - Пурист с Sparse Rewards)')
