...
def setup_gpu_support():
    ...
    if system == "windows":
        try:
            ...
            if torch_directml.is_available(): device = torch_directml.device(); print(f"✅ DirectML найден: {device}"); return device
...
"""
🚀 ТОРГОВАЯ СИСТЕМА V12.0 - МНОГОМЕРНЫЙ СТРАТЕГ
...
✅ ГЛУБОКИЙ КОНТЕКСТ: Агент теперь видит не только локальные 5-минутные паттерны,
   но и глобальный тренд, моментум и волатильность со старших ТФ, что позволяет
   принимать на порядки более взвешенные решения.
"""
...
class MTFDataLoader:
    ...
    def load_and_prepare_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        # Сначала извлекаем нужные колонки, потом переименовываем
...
def main():
    print("🚀 СИСТЕМА V12.0 (Многомерный Стратег) - ЗАПУСК")
    device = setup_gpu_support(); get_gpu_memory_info(device)
    
    ### ИЗМЕНЕНИЕ: Указываем правильные пути к вашим файлам данных
    data_paths = {
    ...
    plt.title(f'Результаты на тестовой выборке (V12.0 - Многомерный Стратег)\nReturn: {total_return:.2f}% | Trades: {trades} | Win Rate: {win_rate:.1f}%')
