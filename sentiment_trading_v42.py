"""
🚀 ОПТИМИЗИРОВАННАЯ ТОРГОВАЯ СИСТЕМА V4.2 - УЛУЧШЕННОЕ ОБУЧЕНИЕ
✅ Исправлена проблема мгновенного завершения эпизода.
✅ Улучшена система вознаграждений (Reward Shaping) для более стабильного обучения.
✅ Снижен риск на сделку для предотвращения "самоубийства" агента.
"""
...
class OptimalConfig:
    ...
    WINDOW_SIZE = 50
    TOTAL_TIMESTEPS = 150000 # Для сложных задач можно увеличить до 200-250k
    LEARNING_RATE = 3e-4
    # ИЗМЕНЕНО: Немного увеличиваем энтропию для поощрения исследований
    ENTROPY_COEF = 0.02

    # ИЗМЕНЕНО: Добавляем штраф за бездействие
    HOLD_PENALTY = -0.01 # Штраф за каждый шаг бездействия
...
class TradingEnv(gym.Env):
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, bool, Dict[str, Any]]:
        ...
        if action == 1 and self.position_amount == 0:
            ...
            # Штраф за комиссию уже учтен в балансе, основная награда будет при закрытии
        elif action == 2 and self.position_amount > 0:
            reward = self._close_position(current_price)
        
        # ИЗМЕНЕНО: Reward Shaping. Добавляем награду за удержание прибыльной позиции
        unrealized_pnl = 0
        if self.position_amount > 0:
            unrealized_pnl = (current_price - self.entry_price) * self.position_amount
            # Если позиция в плюсе, даем небольшой бонус
            if unrealized_pnl > 0:
            ...
            if price_change <= -self.cfg.STOP_LOSS or price_change >= self.cfg.TAKE_PROFIT:
                reward += self._close_position(current_price) # Добавляем PnL к награде
    ...
    def _close_position(self, price: float) -> float:
        ...
        # Комиссия за вход уже была вычтена, теперь вычитаем комиссию за выход
...
def main():
    ...
    # ИЗМЕНЕНО: Явно указываем архитектуру для policy и value function, это хорошая практика
    policy_kwargs = dict(
    ...
    plt.title('Результаты на тестовой выборке (V4.2)')
    ax1 = plt.gca()
