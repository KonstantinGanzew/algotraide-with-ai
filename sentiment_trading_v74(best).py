...
"""
🚀 ТОРГОВАЯ СИСТЕМА V13.2 - ГИБРИДНАЯ СИСТЕМА ВОЗНАГРАЖДЕНИЯ
✅ ЦЕЛЬ: Подавить "шум" в вознаграждении, отделив главный сигнал (реализованная прибыль) от второстепенного (удержание позиции).
✅ КЛЮЧЕВЫЕ ИЗМЕНЕНИЯ:
   1. ГИБРИДНАЯ НАГРАДА:
      - БОЛЬШАЯ НАГРАДА: Агент получает значительную награду/штраф, равную PnL, ТОЛЬКО в момент закрытия сделки.
      - ШТРАФ ЗА УДЕРЖАНИЕ УБЫТКА: На каждом шаге, находясь в убыточной позиции, агент получает небольшой штраф, чтобы не "пересиживать" плохие сделки.
   2. СТАБИЛИЗАЦИЯ НАБЛЮДЕНИЙ: `pnl_norm` в состоянии теперь обрезается (clip), чтобы предотвратить выбросы и дестабилизацию сети.
✅ ОЖИДАЕМЫЙ РЕЗУЛЬТАТ: Агент сфокусируется на поиске сделок с высоким PnL, а не на ловле рыночного шума. Количество сделок должно стать еще меньше и осмысленнее.
"""

# CustomCombinedExtractor и TrendTraderConfig без изменений
...
class TradingEnv(gym.Env):
    ...
    def _get_observation(self)->Dict[str,np.ndarray]:
        ...
        if self.position_amount!=0:
            ...
            # ИЗМЕНЕНИЕ 1: Обрезаем pnl_norm для стабильности сети
    ...
    def step(self,action:int):
        # ИЗМЕНЕНИЕ 2: Новая гибридная функция вознаграждения
        realized_reward = 0.0
        ...
        if current_pos!=desired_pos:
            ...
            if current_pos!=0: realized_reward += self._close_position(price)
        ...
        # Рассчитываем итоговую награду для этого шага
        holding_penalty = 0.0
        if self.position_amount != 0 and unrealized_pnl < 0:
            # Штраф пропорционален нереализованному убытку
    ...
    def _open_position(self,price,is_long):
        ...
        order_size=self.equity*self.cfg.ORDER_SIZE_RATIO # Используем equity для расчета размера, чтобы реинвестировать прибыль
        if self.balance > order_size: # Проверяем, что на балансе достаточно средств
    ...
    def _close_position(self,price):
        # ИЗМЕНЕНИЕ 3: Функция теперь возвращает PnL для использования в награде
        ...
        self.balance+=net_close_value if is_long else (entry_value+pnl)
        self.trades.append(pnl)
        self.position_amount,self.entry_price=0.0,0.0
        return pnl # Возвращаем чистый PnL

def main():
    print("🚀 СИСТЕМА V13.2 (Гибридная Награда) - ЗАПУСК")
    ...
    print("\n🎓 ЭТАП 4: ОБУЧЕНИЕ 'АНАЛИТИКА С ГИБРИДНОЙ НАГРАДОЙ'..."); model.learn(total_timesteps=TrendTraderConfig.TOTAL_TIMESTEPS)
    ...
    plt.style.use('seaborn-v0_8-darkgrid'); plt.figure(figsize=(15,7)); plt.title(f'V13.2 - Гибридная Награда\nReturn: {total_return:.2f}%|Trades:{trades}|Win Rate:{win_rate:.1f}%')
    ...
    ax1.legend(loc='upper left'); ax2.legend(loc='upper right'); plt.savefig('results_v13.2.png'); plt.close(); print("✅ График сохранен в 'results_v13.2.png'")
